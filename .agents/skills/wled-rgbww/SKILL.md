...
9. For exposure selection, use `python3 camera_gphoto2.py auto-expose ... --target-max 49152`; it starts from the current bounded shutter speed, uses decoded max as a hard safety limit, uses the strongest per-channel `p99.9 - p10` contrast as exposure feedback, applies bounded EV jumps for very dark or saturated measurements, refines safe/overexposed brackets by geometric midpoint, caps total shutter releases at `--max-captures` (`10` by default), deletes trial RAW/decoded outputs, and only saves a final capture whose decoded channel max is at or below the target.
10. For WLED code/duty response measurement, use `python3 measure_channel_response.py`; it turns on one ESPHome channel at a time through `esphome.light(...)`, runs bounded auto exposure, decodes `.npy`, normalizes region signal by shutter seconds, optionally subtracts an all-off ambient capture, and writes incremental JSON under `tmp/channel-response/`. Use `--dry-run` first and pass `--allow-high-output` only after confirming the sweep is safe.
11. For color-block location annotation, use `python3 location_picker_ui.py --blocks <count> --rows <rows> --cols <cols>`; it serves a local Web UI on `127.0.0.1:8765`, auto-exposes first with `--max-exposure-trials` defaulting to `3`, stores final capture data under `tmp/location-ui/`, and saves confirmed quadrilateral locations to `config/location/`. Use `--ui-port <port>` for the browser UI port, and `--port` or `--camera-port` for the gphoto2 camera port. If the camera is offline or auto-exposure fails before an image loads, use the toolbar `重试相机` button after reconnecting the camera. With `opencv-contrib-python-headless` installed, the auto-detect button uses OpenCV `mcc` for 4x6 Macbeth 24 ColorChecker detection first, then falls back to OpenCV Canny/Hough grid-angle detection, then to the conservative projection detector.
